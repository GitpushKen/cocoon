...
function start() {
    ...
    setTimeout(function(){titleScreen.classList.toggle("inactive")},550);
    setTimeout(function(){mainMenu.classList.toggle("active")},550);
...
function displayPres() {
    ...
    setTimeout(pres.style.display = "flex", 550);
}

function whatOff() {
    what.classList.remove("active");
    what.classList.toggle("down");
    setTimeout(what.style.display = "none", 550);
}

function howOff() {
    how.classList.remove("active");
    how.classList.toggle("down");
    setTimeout(how.style.display = "none", 550);
}
...
function toggleWho() {
    if (!whoOn) {   
        ...
        setTimeout(what.style.display = "flex", 550);
        how.classList.remove("down");
        setTimeout(how.style.display = "flex", 550);
        who.classList.remove("active");
        pres.classList.remove("active");
        setTimeout(pres.style.display = "none", 550);
        whoOn = false;
        console.log("who",whoOn);
    }
